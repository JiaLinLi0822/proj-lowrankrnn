...
class RankRNNBase(nn.Module):
    def __init__(self, input_size,
        ...
        if torch.cuda.is_available():
            ...
            self.device = torch.device('cpu')  # Changed from string to torch.device

    def forward(self, x, return_dynamics=False):
        alpha = 0.2
        seq_len = x.shape[1]
        batch_size = x.shape[0]
        # Get the actual device of the model (may differ from self.device if model was moved)
        device = next(self.parameters()).device
        if return_dynamics:
            trajectories = torch.zeros(batch_size, seq_len, self.hidden_size, device=device)
        ...
        # simulation loop
        if self.initial_state is not None:
            # Ensure initial_state is on the correct device
            h = self.initial_state.to(device)
        else:
...
class LowRankRNN(RankRNNBase):
    ...
    def get_rec_matrix(self):
        # Ensure the matrix is on the same device as the model parameters
        device = next(self.parameters()).device
        return self.n.matmul(self.m.t()).to(device)
