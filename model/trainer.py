...
def custom_loss(target, predict, gpu=True):
    ...
    is_gpu_available = gpu and (torch.cuda.is_available() or 
                                (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()))
    if is_gpu_available:
        # MPS doesn't support float64, use float32 instead
        # Use clone().detach() to avoid warning about tensor construction
        mask = temp.clone().detach().to(torch.float32).requires_grad_(True)
    else:
        mask = temp.clone().detach().type(torch.FloatTensor).requires_grad_(True)
...
class PyTorchTrainer:
    ...
    def train(self, model):
        ...
        # Use self.epochs instead of hardcoded 3000
        # First phase: train up to min(3000, self.epochs) epochs
        max_epochs_phase1 = min(3000, self.epochs)
        for epoch in range(1, max_epochs_phase1 + 1):
            ...
            if self.check_epoch(early_stopping, epoch, optimizer.param_groups[0]['lr']):
                model.load_state_dict(torch.load(self.model_dir + '/weights.pt', weights_only=False))
                self.train_on = False
                ...
                try:
                    loss_history = {
                        ...
                        'epochs': list(range(1, len(average_losses) + 1))
                    }
                    dump_pickle(self.model_dir + '/loss_history.pkl', loss_history)
        ...
        if epoch >= self.epochs:
            print(f'Reached maximum epochs ({self.epochs}), stopping training')
        ...
        # Second phase: continue training but respect max epochs
