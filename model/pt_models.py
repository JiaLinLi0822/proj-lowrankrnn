...
class PTModelArchitecture(RNNModel):
    ...
    def get_weights(self):
        weights = torch.load(self.model_dir + '/weights.pt', map_location='cpu', weights_only=False)
    ...
    def initial_weights(self):
        weights = torch.load(self.model_dir + '/initial_weights.pt', map_location='cpu', weights_only=False)
        weights = {key.replace('rnn.rnn.', 'rnn.'): val for key, val in weights.items()}
    ...
    def assign_weights(self, model, weights=None):
        ...
        # This handles cases where saved weights have different bias settings
        ...
        for key, value in weights.items():
            if key in model_state_dict:
                ...
                if model_state_dict[key].shape == value.shape:
                    ...
                    print(f"Warning: Shape mismatch for {key}: model expects {model_state_dict[key].shape}, got {value.shape}")
            # Skip keys that don't exist in model (e.g., bias when model doesn't have bias, or vice versa)
    ...
    def predict(self, x, initial_states=None, weights=None):
        ...
        # Get the device the model expects (before creating model)
        ...
        # Move initial_states to target device if provided
        if initial_states is not None:
            if isinstance(initial_states, np.ndarray):
                initial_states = np2torch(initial_states).to(target_device)
            elif isinstance(initial_states, torch.Tensor):
                initial_states = initial_states.to(target_device)
        
        model = self.create_model(initial_state=initial_states)
        
        # Move model to the target device
        model = model.to(target_device)
        
        # Ensure initial_state in model is also on the correct device
        if hasattr(model, 'initial_state') and model.initial_state is not None:
            model.initial_state = model.initial_state.to(target_device)
        
        self.assign_weights(model, weights)
        # Ensure input data is on the same device as the model
        x_torch = np2torch(x).to(target_device)
        pred = model.forward_states(x_torch)
