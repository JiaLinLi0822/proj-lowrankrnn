...
# from descartes import PolygonPatch  # Not used, removed to avoid dependency
import numpy as np
sns.set()
# sns.set_palette("viridis")
# from shapely.geometry import Point  # Replaced with matplotlib Path
# from shapely.geometry.polygon import Polygon  # Replaced with matplotlib Path
plt.rcParams['axes.facecolor'] = 'white'
...
def points_to_attractor(n_points, contours, final_kappas):
    ...
    for index in range(n_points**2):
        point = final_kappas[index]
        # Convert point to tuple for contains_point
        point_tuple = tuple(point) if isinstance(point, (list, np.ndarray)) else point
        for i, contour_path in enumerate(contours):
            # Use matplotlib Path.contains_point instead of shapely
            # contains_point expects a tuple (x, y) or array-like
            try:
                if contour_path.contains_point(point_tuple):
                    matrix[index // n_points, index % n_points] = i
                    break
            except (TypeError, AttributeError):
                # Fallback: use contains_points (plural) which accepts arrays
...
def get_contours(U, V, Z):
    ...
    # Use matplotlib Path instead of shapely Polygon
    contours = [Path(contour) for contour in (gen.lines(0.2))]
    return contours
...
def plot_contours(contours, matrix, eigvals):
    ...
    for i, contour_path in enumerate(contours):
        color = cmap(i)
        if len(contours) == 9:
            # Calculate centroid manually since we're using Path instead of Polygon
            vertices = contour_path.vertices
            if len(vertices) > 0:
                centroid_x = np.mean(vertices[:, 0])
                centroid_y = np.mean(vertices[:, 1])
                if (np.abs(centroid_x) < 2 or np.abs(centroid_y) < 2):
                    color = 'lightgrey'
        # contour_path is already a Path, so use it directly
        patch = PathPatch(contour_path, color=color, alpha=0.5)
