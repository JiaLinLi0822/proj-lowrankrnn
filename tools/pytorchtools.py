...
def get_device():
    ...
    """
    # Check for CUDA (NVIDIA GPU) - Linux/Windows
    if torch.cuda.is_available():
        device = torch.device("cuda")
        print("GPU is available (CUDA)")
        return device
    
    # Check for MPS (Metal Performance Shaders) - Apple Silicon Mac
    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
        device = torch.device("mps")
        print("GPU is available (MPS - Apple Silicon)")
        return device
    ...
    def __call__(self, val_loss):
        ...
        if self.best_score is None:
            ...
            if self.counter >= self.patience:
                ...
                self.model.load_state_dict(torch.load(self.path + '/weights.pt', weights_only=False))
        else:
            self.best_score = score
